...
#        print(p_r_norm)
    ...
    def choose_best_split_point(self, pdf, y, y_prob):
        N = len(pdf)
        M = len(pdf[0])        
        max_gain = -1e50
        best_attr = None
        ...
        for attr in xrange(M):
            ...
            y_prob_scalar = []
            for i in xrange(N):
                label_left_sum[y[i]] = label_left_sum.get(y[i],0) + y_prob[i]
                p_left_sum += y_prob[i]
                y_prob_scalar.append( sum(pdf[i][attr].values()) )
            p_sum = max(p_left_sum, 1e-50)
            #print( y )
            #print( p_left_sum,p_right_sum )
            #print( (label_left_sum,label_right_sum) )
            # triple: (tri_point, tri_y, tri_p)
            events = {}
            for i in xrange(N):
                tri_y = i
                for (point,prob) in pdf[i][attr].items():
                    if not events.has_key(point):
                        events[point] = []
                    events.get(point).append( (tri_y,prob) )
            for (point,value) in sorted( events.items() ):
                for (tri_y, tri_p) in value:
                    tri_label = y[tri_y]
                    step = y_prob[tri_y]*tri_p/max(y_prob_scalar[tri_y],1e-50)
                    p_left_sum -= step
                    p_right_sum += step
                    p_left_sum = max(0, p_left_sum)
                    p_right_sum = max(0, p_right_sum)
                    label_left_sum[tri_label] = label_left_sum.get(tri_label,0)-step
                    label_right_sum[tri_label] = label_right_sum.get(tri_label,0)+step
                    #print(point, tri_label, tri_p )
                temp_gain = 0
                for p_i in self.dict_norm(label_left_sum).values():
                    temp_gain += p_i*math.log(max(p_i,1e-50), 2.0) * p_left_sum/p_sum
                for p_i in self.dict_norm(label_right_sum).values():
                    temp_gain += p_i*math.log(max(p_i,1e-50), 2.0) * p_right_sum/p_sum
                if max_gain < temp_gain:
                    max_gain = temp_gain
                    best_attr = attr
                    best_value = point

                #print(point, temp_gain, p_left_sum, p_right_sum, p_sum)
#        print("choose attr {0} value {1} to split real info is {2}.".format(
#            best_attr,best_value, self.info_gain(pdf,y,y_prob, best_attr,best_value),
#            ))
        return (best_attr, best_value, max_gain)
...
#                    best_value = value
        ...
        (best_attr, best_value, max_gain) = self.choose_best_split_point(pdf, y, label_prob)
...
#        print( label_prob_left )
        ...
        if self.debug:
            print('choose attr {0} to split value is {1} my size is {2} info is {3}'.format(
                best_attr, best_value, len(pdf), max_gain) )
